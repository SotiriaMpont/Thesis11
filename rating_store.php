...
if (count($result1) == 0) {
    ...
    if (count($result0) == 0) {
        ...
        $sql2 = "SELECT * FROM RatingFromStore WHERE id_di=? AND dat_shift=? AND id_aitim=?";
        ...
        if (count($result2) > 0) {
            ...
            $sql3 = "UPDATE RatingFromStore SET speed=?, accuracy=?, customer_service=? WHERE id_di=? AND dat_shift=? AND id_aitim=?";
            ...
            $sql4 = "INSERT INTO RatingFromStore (id_di, dat_shift, id_aitim, speed,accuracy,customer_service) VALUES (?, ?, ?, ?,?, ?)";
